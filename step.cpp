...
#define CHECK_INTERVAL 1000  // 1 second for rest check
...
void loop() {
  ...
  if (currentTime - lastCheckTime >= CHECK_INTERVAL) {
    ...
    float x = ax / 16384.0;  // For ±2g range
    float y = ay / 16384.0;
    float z = az / 16384.0;

    // Calculate differences from last position
    float dx = x - lastX;
    float dy = y - lastY;
    float dz = z - lastZ;
    ...
    // Calculate movement magnitude (absolute differences)
  ...
  if (!dataSent && currentTime - lastWiFiScanTime >= WIFI_SCAN_INTERVAL) {
    ...
    int n = WiFi.scanNetworks();
    for (int i = 0; i < n; ++i) {
      if (WiFi.SSID(i) == ssid && WiFi.RSSI(i) >= -50) { // -30 to -35 dBm ~ 1cm proximity
